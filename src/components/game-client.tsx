...
export default function GameClient() {
  ...
  const startGame = useCallback(async () => {
    ...
    setPrizeWon(0);
    try {
        const newQuestions = await generateQuiz(currentUser?.uid);
  ...
  }, [toast, isAiConfigured, isProcessing, currentUser]);
  ...
  const advanceToNextQuestion = useCallback((newIndex: number) => {
